...
- `pocketmine/bedrock-data`, `pocketmine/binaryutils`, `pocketmine/callback-validator`, `pocketmine/color`, `pocketmine/errorhandler`, `pocketmine/log`, `pocketmine/math`, and `pocketmine/nbt` are local Composer path packages.
...
- Full repository PHPUnit, PHPStan, PHP-CS-Fixer 3.75 dry-run, code generation, translation validation, and `git diff --check` are clean after the `bedrock-data` import.
- Fork CI health is the top active gate; the latest checked stable runs were green before the `bedrock-data` import.
...
- [x] Preserve source links and authorship; do not copy whole discussions.
  ...
  - The preselected shortlist candidates have been reviewed. Refresh and read original items before adding another; the next ready non-backlog task is the focused Bedrock item upgrade schema dependency import.

## Dependency Track

Completed: `bedrock-data`, `binaryutils`, `callback-validator`, `color`, `errorhandler`, `log`, `math`, `nbt`.
...
1. `bedrock-item-upgrade-schema`
2. `bedrock-block-upgrade-schema`
3. `bedrock-protocol`
4. `snooze`, `raklib`, `raklib-ipc`
...
- **Self-contained dependencies: active.** Eight packages are local; upgrade schema, protocol, and network packages remain.
