...
function shortRef(?string $ref) : string{
	return $ref === null ? "-" : substr($ref, 0, 9);
}
...
function writeMarkdown(string $path, array $results) : void{
	$groups = [
		SourceKind::ROOT->value => "Canonical Root",
		SourceKind::IMPORTED_PACKAGE->value => "Imported PMMP Packages",
		SourceKind::COMPOSER_PACKAGE->value => "Composer-Managed PMMP Packages",
		SourceKind::PROTOCOL_REFERENCE->value => "Protocol References"
	];
	$lines = [
	...
	foreach($groups as $kind => $heading){
		$matching = array_values(array_filter($results, static fn(AuditResult $result) : bool => $result->source->kind->value === $kind));
		if(count($matching) === 0){
			continue;
		}
		$lines[] = "## $heading";
		$lines[] = "";
		$lines[] = "| Source | Branch | Pinned | Latest head | License | Signal |";
		$lines[] = "|:--|:--|:--|:--|:--|:--|";
		foreach($matching as $result){
			$name = "[" . markdown($result->source->name) . "](" . $result->url . ")";
			$lines[] = sprintf(
				"| %s | `%s` | `%s` | `%s` | `%s` | `%s` |",
				$name,
				markdown($result->branch),
				shortRef($result->source->pinnedRef),
				shortRef($result->latestRef),
				markdown($result->license),
				$result->signal
			);
		}
		$lines[] = "";
...
try{
	...
	writeMarkdown($outputDir . "/report.md", $results);
	...
	file_put_contents($outputDir . "/report.json", $json . PHP_EOL);
